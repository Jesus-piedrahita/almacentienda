...
description: >
  ...
  Trigger: When updating skill metadata (metadata.scope/metadata.auto_invoke), regenerating Auto-invoke tables, or running .opencode/skills/skill-sync/assets/sync.sh (including --dry-run/--scope).
license: Apache-2.0
metadata:
  author: jesusp
  version: "1.0"
...
1. Reads all `.opencode/skills/*/SKILL.md` files
...
```yaml
# .opencode/skills/react-19/SKILL.md
metadata:
  author: jesusp
  version: "1.0"
  scope: [ui]
  auto_invoke: "Creating React 19 components"
```
...
| Action | Skill |
|--------|-------|
| Creating React 19 components | `react-19` |
```
...
- [ ] Ran `./.opencode/skills/skill-sync/assets/sync.sh`
